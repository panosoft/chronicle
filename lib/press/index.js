...
var create = function (options) {
	...
	var run = suspend.promise(function * (definition, parameters) {
		...
		if (_.isString(definition)) {
			// handle case where definition is a non-prefixed relative path
			// (i.e. `index.js` vs `./index.js`) by resolving it to an absolute
			// path since fileLoader does not handle it non-prefixed relative
			// paths
			if (localPath(definition)) definition = path.resolve(definition);
